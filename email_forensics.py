...
@author: Oluwafemi Samuel, Elizabeth Ighodaro

A simple open source email forensic component that can be integrated with an enterprise level application for email fraud detection.
For inquiries or high level integration consulting, please contact Oluwafemi Samuel (so.oluwfemi@gmail.com) or Ighodaro Elizabeth (eajunam@gmail.com).
...
#Can be generified to accept all email file formats. The scope of this work accepts eml extension.               
def trackEmail():
    ...
    try:
        ...
        print('Invalid file name. Please verify file name and try again')
        
#Looks up mail source for tracking purpose . An open source api is used to get information about ip. This can be extended/modified to accomodate client needs 
def ipLookup():
...
#code words may be used by suspects to convey special messages between themselves. This piece looks for such words and aids forensic investigation.    
def frequentWords():
    extractWords()
